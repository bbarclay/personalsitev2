...
# Script to update all math pages to properly import and use their index.tsx files
...
function update_page() {
  ...
  local component_file="$dir_path/components/index.tsx"
  
  if [[ ! -f "$component_file" ]]; then
    echo "Warning: No index.tsx found in $dir_path/components"
    return
  fi
  
  if grep -q "import.*from.*components.*index" "$page_file"; then
  ...
  # Get component export name from index.tsx
  local export_line=$(grep -E "export default |export const |export function" "$component_file" | head -1)
  local exported_name=""
...
import { $exported_name } from './components';
...
# Process all math directories with index.tsx files
for component_file in $(find "$BASE_DIR" -path "*/components/index.tsx" | grep -v "node_modules" | grep -v "/slot-machine/components/"); do
