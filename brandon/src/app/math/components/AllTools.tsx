...
export function AllTools({ pages }: AllToolsProps) {
  if (pages.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 text-center">
        <p className="text-gray-500 dark:text-gray-400">No math tools available.</p>
      </div>
    );
  }
  
  // Organize tools by category
  const categories = pages.reduce<Record<string, MathPageMeta[]>>((acc, page) => {
  ...
  return (
    <section>
      <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-white">All Math Tools</h2>
