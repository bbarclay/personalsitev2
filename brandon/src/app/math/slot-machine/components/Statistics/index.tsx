...
const StatItem: React.FC<StatProps> = ({ label, value, tooltip }) => (
  <div className="bg-gray-800 p-4 rounded-lg" title={tooltip}>
...
export const Statistics: React.FC = () => {
  const { stats, totalSpins, totalWins, balance, betAmount, totalPayout } =
    useGameState();
  ...
  const holdPercentage =
    totalCoinIn > 0 ? ((totalCoinIn - totalCoinOut) / totalCoinIn) * 100 : 0;
  const rtp = totalCoinIn > 0 ? (totalCoinOut / totalCoinIn) * 100 : 0;
  const houseEdge = 100 - rtp;
  const hitFrequency = totalSpins > 0 ? (totalWins / totalSpins) * 100 : 0;
  ...
  const winAmounts: number[] = stats
    .filter((stat: StatItem) => stat.totalPayout > 0)
    .map((stat: StatItem) => stat.totalPayout);
  const mean =
    winAmounts.length > 0
  ...
  const variance =
    winAmounts.length > 0
      ? winAmounts.reduce((a, b) => a + Math.pow(b - mean, 2), 0) /
      winAmounts.length
      : 0;
